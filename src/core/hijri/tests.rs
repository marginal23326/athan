...
fn known_hijri_conversion_ramadan() {
    let hijri =
        HijriDate::from_gregorian(time::Date::from_calendar_date(2026, time::Month::February, 18).unwrap()).unwrap();
...
fn known_hijri_conversion_1447_start() {
    for day in 25..=30 {
        let hijri =
            HijriDate::from_gregorian(time::Date::from_calendar_date(2025, time::Month::June, day).unwrap()).unwrap();
        if hijri.year == 1447 && hijri.month == 1 && hijri.day == 1 {
            return;
        }
    }
    let hijri =
        HijriDate::from_gregorian(time::Date::from_calendar_date(2025, time::Month::June, 27).unwrap()).unwrap();
...
fn is_ramadan_through_year() {
    ...
    for (y, m, d, expected) in &test_cases {
        ...
        assert_eq!(is_ramadan(date), *expected, "{y}-{m:?}-{d} Ramadan detection failed");
    }
}
